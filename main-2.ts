...
const arrTen: any = [100, 'string', true, false, [1, 2, 3], {
    name: 'Andrew',
    surname: 'Grabynsky'
...
class BookAuthor extends Book {
    authors: [{ name: string, age: number | string }];

    constructor(title: string, pageCount: number, genre: string, authors: [{ name: string, age: number | string }]) {
...
const bookJSAuthors: BookAuthor = {
    title: 'Вивчаємо JavaScript',
    pageCount: 361,
    genre: 'Програмування',
...
const bookShevchenkoAuthors: BookAuthor = {
    title: 'Кобзар',
    pageCount: 402,
    genre: 'Поезія',
...
const bookRomanyAuthors: BookAuthor = {
    title: 'Маруся Чурай',
    pageCount: 188,
    genre: 'Романи',
...
const week: { morning: number, day: number, evening: number }[] = [
...
let oneNum1: number = 3;
let twoNum1: number = 3;
if (oneNum1 !== twoNum1) {
    if (oneNum1 > twoNum1) {
        console.log('Перше число більше від другого');
    } else if (oneNum1 < twoNum1) {
...
// let oneNum:number= prompt('enter one number');
// let twoNum:number = prompt('enter two number');
//
// if (oneNum !== twoNum) {
//     if (oneNum > twoNum) {
